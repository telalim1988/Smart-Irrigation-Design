...
function generateReport() {
  ...
  // =========================
  doc.setFontSize(18);
  doc.text("Smart Irrigation AI Design Report", 20, 20);

  doc.setFontSize(10);
  doc.text("Generated by HydroSmart AI", 20, 28);
  ...
  let emitter = document.getElementById("emitter_flow").value;
  ...
  doc.setFontSize(10);
  doc.text(`Area: ${area} m²`, 20, 48);
  doc.text(`Zones: ${zones}`, 20, 54);
  doc.text(`Emitter Flow: ${emitter} L/hr`, 20, 60);
  doc.text(`Operating Hours: ${hours} hr/day`, 20, 66);

  // =========================
  // 🔹 RESULTS
  // =========================
  let flow = document.getElementById("pump_flow").innerText;
  let head = document.getElementById("pump_head").innerText;
  let status = document.getElementById("pump_status").innerText;
  let energy = document.getElementById("energy")?.innerText || "-";

  doc.text("CALCULATION RESULTS", 20, 80);

  doc.text(`Flow: ${flow} m³/hr`, 20, 88);
  doc.text(`Head: ${head} m`, 20, 94);
  doc.text(`Pump Status: ${status}`, 20, 100);
  doc.text(`Energy: ${energy} kWh/day`, 20, 106);

  // =========================
  // 🔹 AI RESULTS
  // =========================
  let opt_d = document.getElementById("opt_diameter")?.innerText || "-";
  let opt_v = document.getElementById("opt_velocity")?.innerText || "-";

  doc.text("AI OPTIMIZATION", 20, 120);

  doc.text(`Optimal Diameter: ${opt_d}`, 20, 128);
  doc.text(`Optimal Velocity: ${opt_v}`, 20, 134);
